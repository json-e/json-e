...
@pytest.mark.parametrize("tokenizer,input,output", tokenizer_tests.values(), ids=tokenizer_tests.keys())
...
def test_parser_diff_precedence():
    input = '3+8*2'
    output = BinOp(Token('+', '+', 1, 2), ASTNode(Token('number', '3', 0, 1)),
